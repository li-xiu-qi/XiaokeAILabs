...
四份报告共内嵌 7 张图，全部由 `figures/make_figures.py` 从结果数据生成（主报告 4 张：召回-延迟帕累托、存储放大倍数、构建时间、写入吞吐；随机向量报告 1 张：SurrealDB/Milvus ef 异常；五库对比 1 张：LanceDB 调参根因；部署能力矩阵 1 张，数据来自已迁知识库的部署形态总览）：
...
| [docs/各库体积与安装包数据.md](docs/各库体积与安装包数据.md) | 七个库在 clone 源码 / pip 包 / Docker 镜像 / 二进制安装包 / 运行时内存五个维度的体积对比（2026-09-05 版本快照，方法论已迁知识库） |

已迁往配套知识库的综述与选型结论（本仓不再维护原文）：分词器分类与选型、jieba 组成环节、向量索引类型原理与选型、六库能力与部署形态选型、六库公司与维护模式背景、混合检索评测方法论、体积测量方法论、RocksDB 与 LSM 背景、六库 BM25 支持度与接入难度、FAISS 预计算表限制、向量搜索底层实现。本仓保留的是复现链：探针与基准脚本、`results/` 原始数据、四份基准报告、调试排错记录。复现不受迁移影响。
