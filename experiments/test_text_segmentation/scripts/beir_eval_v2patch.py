...
# 修复 3：去掉 meta device 初始化上下文（flash 实现的 alibi buffer
# 在 meta 上创建会触发 torch 设备检查错误）
import torch as _torch
_orig_gic = _mu.PreTrainedModel.get_init_context
def _patched_gic(*_args, **_kwargs):
    _ctxs = _orig_gic(*_args, **_kwargs)
    return [c for c in _ctxs if not (isinstance(c, _torch.device) and c.type == "meta")]
_mu.PreTrainedModel.get_init_context = _patched_gic

# 修复 4：补回新版 transformers 已移除的 get_head_mask（flash 实现 forward 依赖）
if not hasattr(_mu.PreTrainedModel, "get_head_mask"):
    def _get_head_mask(self, head_mask, num_hidden_layers, dtype=None):
