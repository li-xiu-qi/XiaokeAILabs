...
三个脚本在 aarch64 GB10（20 核 CPU，121 GB 统一内存，torch 2.14.0+cu130）上实测通过，完整原始日志见 `../docs/logs/`。
...
- **极高的推理速度与多题并行收益**：单题前向 p50 为 8.31 ms（中文走 multilingual）至 16.41 ms（英文走 english），路由判定本身只占 0.02 至 0.19 ms。单次请求合并 16 题时，单题边际延迟摊薄到 2.77 ms，约为单题调用的六分之一，适合高并发极速决策。
- **显存体量轻巧**：三 checkpoint 全量常驻 CUDA 分配 5.96 GB，峰值 6.68 GB，普通单卡环境即可承载。
