...
class CachedEmbeddingModel:
    ...
    def __init__(
        ...
        Args:
            ...
            enable_cache: 是否启用缓存
            api_key: 远程API密钥（可选）
            base_url: 远程API基础URL（可选）
        """
        if base_model is not None:
            self.base_model = base_model
        else:
            self.base_model = load_embedding_model(model_type="api", model_name="BAAI/bge-m3", api_key=api_key, base_url=base_url)
...
def get_cached_embedding_model(
    ...
    Returns:
        CachedEmbeddingModel: 配置好的缓存嵌入模型实例
    """
    return CachedEmbeddingModel(
        base_model=None,  # 使用默认模型
        cache_dir=cache_dir,
