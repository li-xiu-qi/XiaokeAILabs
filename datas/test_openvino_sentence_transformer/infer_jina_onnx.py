...
def run_jina_inference(onnx_model_path: str, device_name: str, tokenizer_path: str, image_processor_path: str, texts: list[str], image_url: str = None, max_len: int = 77):
    """
    使用 OpenVINO Runtime 加载 Jina ONNX 模型并执行推理。

    参数:
        onnx_model_path (str): ONNX 模型文件的路径。
        device_name (str): 推理设备 ("CPU", "GPU", "NPU")。
        tokenizer_path (str): Tokenizer 目录的路径。
        image_processor_path (str): Image Processor 目录的路径。
        texts (list[str]): 用于推理的文本列表。
        image_url (str, optional): 用于推理的图片 URL。默认为 None。
        max_len (int, optional): 文本序列的最大长度。默认为 77。
    """
    ...
    try:
        ...
        try:
            ...
            print(f"成功从 '{tokenizer_path}' 加载分词器: {type(tokenizer)}")
        ...
        try:
            print("正在加载图像预处理器...")
            image_processor = AutoImageProcessor.from_pretrained(image_processor_path, trust_remote_code=True)
            print("图像预处理器加载成功！")
        ...
        try:
            ...
            if text_embeddings is not None:
                ...
                for i in range(num_texts):
                    ...
                    print(f"'{texts[i]}' 最相似的句子是: '{texts[most_similar_idx]}' (相似度: {similarity:.4f})")
            else:
...
if __name__ == "__main__":
    # --- 配置 ---
    onnx_model_path_config = r"C:\\Users\\k\\Desktop\\BaiduSyncdisk\\baidu_sync_documents\\hf_models\\jina-clip-v2\\onnx\\model.onnx"
    device_name_config = "CPU"
    tokenizer_directory_path_config = r"C:\\Users\\k\\Desktop\\BaiduSyncdisk\\baidu_sync_documents\\hf_models\\jina-clip-v2"
    # processor_directory_config = r"./preprocessor" # 假设图像处理器与分词器在同一父目录或特定子目录
    # 为了与函数签名一致，我们假设 image_processor 也在 tokenizer_directory_path_config 下，或者需要单独指定
    # 如果 image_processor 在 tokenizer_directory_path_config 下的 'preprocessor' 子目录，可以这样构造：
    # image_processor_path_config = str(Path(tokenizer_directory_path_config) / "preprocessor")
    # 或者，如果它与 tokenizer 在同一级别（例如，都直接在 jina-clip-v2 下），则路径相同
    image_processor_path_config = tokenizer_directory_path_config # 假设与tokenizer在同一目录
    ...
    sample_image_url_config = "http://images.cocodataset.org/val2017/000000039769.jpg"
    max_sequence_length_config = 77

    run_jina_inference(
        onnx_model_path=onnx_model_path_config,
        device_name=device_name_config,
        tokenizer_path=tokenizer_directory_path_config,
        image_processor_path=image_processor_path_config, # 使用上面定义的路径
        texts=sample_texts_config,
        image_url=sample_image_url_config,
        max_len=max_sequence_length_config
    )
