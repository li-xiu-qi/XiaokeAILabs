...
重要：仔细分析用户的原始请求和已完成的工具执行，确定是否还有未完成的任务。

用户原始请求：{user_message}

根据之前的工具执行结果，决定下一步行动：

选项1 - 如果还有未完成的任务，必须继续执行工具：
Thought: [分析还需要完成什么任务]
Action: [工具名称] [参数JSON]

选项2 - 只有当所有任务都已完成时，才能给出最终答案：
Thought: [确认所有任务已完成，基于工具结果的分析]
Final Answer: [基于真实工具结果的完整答案]

绝对不能编造任何信息！所有信息必须来自工具的真实执行结果。"""
