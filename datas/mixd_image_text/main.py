...
if user_input and user_input.strip():
    ...
    with st.status("正在处理内容，请稍候...", expanded=True) as status: # Changed status message
        try:
            status.write("📄 正在加载参考内容...") # Changed status message
      
            answer_blocks = get_reference_content()
            status.write(f"✅ 内容加载完成。") # Changed status message

            if answer_blocks: 
                status.write("🤖 正在调用大模型流式生成回答...")
                chat_history = st.session_state["history"][-5:]
                response = call_guiji_rag_model_stream(
                    user_input, chat_history # Pass None for the removed parameter
                )
                full_answer = ""
